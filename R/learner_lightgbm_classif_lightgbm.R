...
# Slightly modified in order to reduce dependencies
# Copied on 29.01.2025
################################################################################
...
LearnerClassifLightGBM = R6::R6Class("LearnerClassifLightGBM",
                                 ...
                                 private = list(
                                   ...
                                   .predict = function(task) {
                                     ...
                                     pred = do.call(predict, args=list(self$model,
                                                                       data,
                                                                       params = pars))
...
################################################################################
# COPIED FROM: https://github.com/mlr-org/mlr3extralearners/blob/main/R/encode_data_lightgbm.R
# (contains the  package [mlr3extralearners] developped by Raphael Sonabend, Patrick Schratz, Sebastian Fischer.)
# Copied on: 29.01.2025
################################################################################
...
################################################################################
# COPIED FROM: https://github.com/mlr-org/mlr3extralearners/blob/main/R/helpers.R
# (contains the  package [mlr3extralearners] developped by Raphael Sonabend, Patrick Schratz, Sebastian Fischer.)
# Copied on 29.01.2025
# Slightly modified in order to reduce dependencies
################################################################################
...
ordered_features = function(task, learner) {
  ...
  if(!is.null(names(learner$state$data_prototype))){
    cols <-  names(learner$state$data_prototype)
  }
  else{
    cols <- learner$state$feature_names
  }
